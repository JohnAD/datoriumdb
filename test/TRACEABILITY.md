...
| --- | --- | --- | --- |
| Access-language CRUD | ACCESS-LANGUAGE.md | unit: `internal/engine`, `internal/commandreq`; contract: `test/contract` create/read/patch/delete + error-path golden envelopes (`distributionComplete` on write success); integration: `TestSingleNodeCRUDLifecycle` | covered |
| HTTP command transport | ACCESS-LANGUAGE.md, LOCAL-ARCHITECTURE.md | unit: `internal/server` JSON/multipart `/command` (legacy text/`/files` removed); integration: `TestSingleNodeCRUDLifecycle`, `TestSingleNodeUnauthenticatedCommandRefused`; compose: `TestComposeSingleNodeCRUD` | covered |
...
| Binary attachments | BINARY-FILES.md | unit: `internal/fsstore` (`files*`), `internal/replication` (`file_*`); HTTP: `internal/server` (`TestFileHTTPLifecycle`, `TestFileHTTPInvalidFileName`, `TestFileHTTPLargeUploadAndReadStream`, `TestReplicationHappyPathFileCreateUpdateDelete`); contract: `file_create_ok`, `file_exists`, `file_list_ok`, `file_invalid_name`; clients: Go `files_test.go`, Python `tests/unit/test_files.py` (JSON/multipart `/command`) | covered |
...
| Precompiled search MVP | SEARCHING.md, SEARCH-DEFINITION-SCHEMA.md, SEARCHING-V1.md | unit: `internal/search` (definition validation including array `contains`, evaluate multi-bucket, path encoding, sort/null/missing, store idempotency); engine search command; change-agent bucket move/delete + contains membership (`TestChangeAgentContainsMultiBucketMembership`); integration: search replication to read member; compose search lifecycle still thin | covered |
| Admin catalog ensure (collections/searches) | docs/api.md, AUTHENTICATION.md | unit: `internal/adminconfig`, `internal/server` (`http_admin_test`), `internal/engine` admin commands, `internal/ctl` HTTP mutators; clients: Go `EnsureCollection` / Python `ensure_collection` | covered |
